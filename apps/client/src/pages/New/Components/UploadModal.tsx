...
import { handleFileUpload } from "@sh/lib/handlers/handleFileUpload";

export const UploadModal = ({isOpen, setOpen, userId, title, prompt, docs, setDocs}) => {
  ...
  async function handleUpload(e, userId) {
    e.preventDefault();
    setError(null)
    const result = await handleFileUpload(e.target.files[0], userId, setLoading, setError);
    handleReload(result)
  }
  async function handleDataTransferUpload(e) {
    e.preventDefault();
    setError(null)
    const result = await handleFileUpload(e.dataTransfer.files[0], userId, setLoading, setError);
    handleReload(result)
  }
